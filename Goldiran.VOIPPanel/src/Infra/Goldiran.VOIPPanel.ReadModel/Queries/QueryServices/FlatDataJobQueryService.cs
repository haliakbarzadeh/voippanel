...
public class FlatDataJobQueryService : IFlatDataJobQueryService
{
    private VOIPPanelReadModelContext _context;
    private readonly IMapper _mapper;
    public FlatDataJobQueryService(VOIPPanelReadModelContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }
    public async Task<FlatDataJobDto> GetLastFlatData(GetFlatDataJobLastQuery filter)
    {
        var entity=await _context.FlatDataJobs.AsNoTracking()
            .Where(c=>filter.ReportType!=null?c.ReportType==filter.ReportType:true)
            .OrderBy(c=>c.Id).LastOrDefaultAsync();
        return _mapper.Map<FlatDataJobDto>(entity);
    }
