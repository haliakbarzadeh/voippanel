...
namespace Goldiran.VOIPPanel.Domain.Common.Contracts
{
    public interface IAppUserManager : IDisposable
    {
        ...
        Task<AppUser> FindByUserIdAsync(long UserId);
        ...
        /// <summary>
        /// Normalize a key (AppUser name, email) for consistent comparisons.
        /// </summary>
        ...
        Task<bool> HasPhoneNumberAsync(long userId);
        ...
        Task<AppUser> FindByIdIncludeUserRolesAsync(long userId);

        Task<IdentityResult> UpdateUserAndSecurityStampAsync(long userId, Action<AppUser> action);

        Task<IdentityResult> AddOrUpdateUserRolesAsync(long userId, IList<long> selectedRoleIds, Action<AppUser> action = null);

        //Task<PagedUsersListResponse> GetPagedUsersListAsync(GetUsersByFilterQuery model);
        ...
        Task<IdentityResult> RemoveClaimsAsync(long userId, IEnumerable<Claim> claims);
