...
public class CreateOperationSettingCommand : BaseCreateCommandRequest, IRequest<int>
{
    ...
    public class Handler : IRequestHandler<CreateOperationSettingCommand, int>
    {
        ...
        public async Task<int> Handle(CreateOperationSettingCommand request, CancellationToken cancellationToken)
        {
            var entity = new OperationSetting((OperationType)request.OperationTypeId, request.Name, request.Description, request.Color, request.Icon, request.Label, (int)request.Order, request.InQueue, request.IsActive, request.Pasuse, request.ShowToUser, request.HitLimit, request.Duration, request.Penalty);
            ...
            _operationSettingRepository.Add(entity);
